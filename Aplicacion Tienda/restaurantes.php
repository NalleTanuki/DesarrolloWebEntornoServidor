<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['add'])) {      //Si lo que se recibe por POST es un "anhadir"
            $correo = $_POST['correo'];
            $clave = password_hash($_POST['clave'], PASSWORD_BCRYPT); // Ciframos la clave y asi se almacena ya cifrada
            $pais = $_POST['pais'];
            $cp = $_POST['cp'] ?? null; // si NO se rellena, lo guardamos como "null" ya que no es un campo boligatorio
            $ciudad = $_POST['ciudad'];
            $direccion = $_POST['direccion'];
            $rol = $_POST['rol'];
            insertar_restaurante($correo, $clave, $pais, $cp, $ciudad, $direccion, $rol);
            ...
            $direccion = $_POST['direccion'];
            $rol = $_POST['rol'];
            editar_restaurante($correo, $clave, $pais, $cp, $ciudad, $direccion, $rol, $codRes);
        } elseif (isset($_POST['delete'])) { // Si es para eliminar
            $codRes = $_POST['codRes'];
            eliminar_restaurante($codRes);
        }
    }
